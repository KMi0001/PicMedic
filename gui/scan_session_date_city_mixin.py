...
gui/scan_session_window.py::ScanSessionWindow의 일부. 정리 허브의 "날짜별 정리"
/ "도시별 정리" 카드 전환 + 두 화면이 함께 쓰는 그룹 상세(date_group_detail_screen)
로직. 둘 다 같은 그룹 상세 화면·시그니처(set_group_excluded(label, excluded_paths))를
공유해서 한 파일에 같이 뒀다.
