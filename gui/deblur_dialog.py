...
"디블러/디노이즈"(core/deblur.py) 실행 흐름 — gui/face_restore_dialog.py와
같은 구조(확인 팝업 → 진행 팝업 → 원본/결과 2분할 결과 안내). 얼굴 전용이
아니라 사진 전체에 적용되고, 예상 소요 시간은 해상도를 알면 정확히 계산할 수
있다는 점만 다르다(core/quality_enhancer.py의 estimate_seconds와 같은 방식).
"""
...
def _confirm_deblur(parent: QWidget, width: int | None, height: int | None) -> bool:
    ...
    message_lines = [
        "손떨림 블러나 노이즈가 있는 사진을 보정해요.",
        "인물이 없는 풍경/사물 사진에도 적용돼요.",
        "원본은 그대로 두고 새 파일로 저장돼요.",
    ]
    if width and height:
        seconds = deblur.estimate_seconds(width, height)
        message_lines += [
            "",
    ...
    confirm_btn = QPushButton("디블러/디노이즈 시작")
...
class _DeblurResultDialog(QDialog):
...
class _DeblurWorker(QThread):
    ...
    def run(self):
        try:
            result_path = deblur.deblur_image(
                self._input_path,
                self._output_dir,
                progress_callback=self.progress.emit,
                should_cancel=lambda: self._cancel_requested,
            )
        except deblur.DeblurCancelled:
            return
...
def run_deblur(parent: QWidget, path: str, width: int | None = None, height: int | None = None) -> None:
    """path 사진을 디블러/디노이즈한다 — 확인 팝업부터 결과 안내까지 전부 처리."""
    if not deblur.is_available():
        info_dialog(parent, "이 기기에서는 디블러/디노이즈 기능을 쓸 수 없습니다.")
        return

    if not _confirm_deblur(parent, width, height):
        return

    output_dir = str(Path(path).parent / "Enhanced")
    progress_dialog = ProgressDialog(parent)
    worker = _DeblurWorker(path, output_dir, parent)
    ...
    def on_succeeded(result_path: str):
        ...
        result_dialog = _DeblurResultDialog(path, result_path, output_dir, parent)
        result_dialog.exec()

    def on_failed(message: str):
        progress_dialog.accept()
        info_dialog(parent, f"디블러/디노이즈에 실패했습니다:\n{message}")
    ...
    progress_dialog.start("디블러/디노이즈 중")
    worker.start()
