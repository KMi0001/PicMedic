...
- **정렬은 그 화면의 나머지 레이아웃을 따른다.** 화면 자체가 가운데 정렬(예: "복구
  완료" 화면 — 카드 하나가 창 가운데 떠 있음)이면 칩 묶음 `QHBoxLayout` 앞뒤에
  `addStretch(1)`을 둘 다 넣어 가운데 정렬. 화면이 왼쪽 정렬(예: 사진 복구/변환
  화면 — 제목·뒤로가기·설정 카드가 전부 왼쪽 붙음)이면 칩도 앞쪽 stretch 없이
  왼쪽으로 맞춘다 — 그렇지 않으면 칩만 가운데 떠서 나머지 레이아웃과 안 맞고
  안정감이 떨어져 보인다. 뒤쪽 `addStretch(1)`은 두 경우 다 유지해서 칩 자체가
  줄어들지 않게 한다.
