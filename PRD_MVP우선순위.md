...
### 9. (신규, UX 개선 제안) 복구/변환 진행 중 다른 설정을 못 건드리게 막고, 중단 버튼 필요
지금 `gui/recovery_screen.py::_start_recovery`는 "Medic!" 버튼 하나만 비활성화하고 나머지(복구 방식 라디오, 형식/화질 드롭다운, 저장 위치, 찾아보기, 파일명 문구, 검증 체크박스, "← 뒤로" 버튼)는 배치 작업이 진행되는 동안에도 계속 조작 가능한 상태로 남아있음. 다수 파일(수천~수만 장) 복구/변환 중에 사용자가 설정을 바꾸거나 뒤로 가면 혼란을 줄 수 있음.

또한 `core/converter.py::recover_batch`에는 스캔(`scan_paths`)과 달리 취소 기능(`should_cancel`)이 아예 없고, `gui/recovery_screen.py`의 `RecoveryWorker`에도 `cancel()` 메서드가 없음 — 한번 시작하면 끝까지 기다리거나 앱을 강제 종료하는 수밖에 없음.

**제안**: 복구 진행 중에는 "Medic!" 버튼뿐 아니라 다른 설정 영역 전체를 비활성화하고, 스캔 화면처럼 "중단" 버튼을 추가해 `should_cancel` 콜백을 `recover_batch`까지 전달. 중단 시 그때까지 처리된 파일은 유지.
