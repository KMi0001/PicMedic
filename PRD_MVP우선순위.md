...
## P3 — 확장 (Phase 2~4, MVP 범위 아님) **[2026-09-16 현황 갱신]**

> 아래는 이 표가 마지막으로 갱신된 뒤(2026-08-31) 실제로 벌어진 일을 반영한 것.
> Phase 2는 사실상 완료됐고, Phase 3(AI)은 "만들었다가 되돌린" 케이스라 원 PRD의
> 제외 목록과 다시 어긋나는 부분이 생겼음 — 아래 각주로 표시.

### Phase 2 — 사진 정리 (31장) — ✅ 거의 완료
- [x] 정확 중복 탐지(SHA-256) + 폴더 단위 일괄 처리 + 자동 "남길 파일" 추천
- [x] 유사 중복 탐지(퍼셉추얼 해시 + 파일명 패턴)
- [x] 날짜별 분류(EXIF 촬영일, 연/월 단위, 뷰어 우선 → 정리 실행 2단계)
- [x] 도시별 분류(GPS→도시, 오프라인 역지오코딩, 지도|목록 화면)
- [x] 자동 폴더 구성 — 날짜/도시별 정리의 "정리 실행"(기본 복사, 이동은 명시적 옵션)이 이에 해당
- [x] 기기 정보(EXIF Make/Model, 상세 화면 표시) — 2026-09-16 구현
- [x] GPS 없는 사진 위치 추론(시간 보간 + 유사 사진 클러스터 전파, `core/location_inference.py`) — 2026-09-16 구현
- 스크린샷 자동 묶기는 별도 구현 안 함 — 카테고리 찾기의 "스크린샷/문서"(CLIP)가 이미 같은 역할
- [ ] 장소 세부 단위(동 단위) — 행정구역 데이터 소스 결정 필요, 보류
- **구조 변화(2026-09-07~13)**: 처음엔 검사 결과 화면과 분리된 별도 "정리 허브" 화면(`gui/organize_hub_screen.py`)이었으나, 2026-09-13에 검사 결과 화면(`gui/result_screen.py`)에 합쳐짐 — 체크박스/확장자변환 표는 그대로 두고 그 아래에 정리 카드를 붙이는 구성. 자세한 내용은 `PHASE2_사진정리_기획.md` 참고.
- 상세 진행 기록은 `PHASE2_사진정리_기획.md` 전체 참고 — 이 표보다 그 문서가 항상 최신.

### Phase 3 — AI 기능 (31, 32장) — 🟡 만들었다가 대부분 되돌림
- [x] → **제거됨(2026-09-13)** 화질 개선(Real-ESRGAN), 얼굴 복원(RestoreFormer++), 디블러/디노이즈(NAFNet), 사진 진단(블러/노출/대비/노이즈 + 얼굴탐지). 이유: CUDA torch가 섞이면 배포 패키지가 5.7GB까지 불어났고 Real-ESRGAN macOS 바이너리도 못 구해서, torch 자체를 앱에서 빼기로 결정.
- [x] **사진 그룹화(자동 사진 분류)는 남음** — 정리 > 카테고리 찾기(동물친구들/음식 사진/스크린샷·문서/야경/풍경 5종, CLIP 기반). PyTorch가 아니라 ONNX Runtime + INT8 양자화로 동작(2026-09-13 전환) — 위 torch 제거와 무관하게 계속 작동. **스크린샷 자동 분류는 이 5종 중 "스크린샷/문서" 카테고리로 커버됨.**
- [ ] 흐림/흔들림/눈감음 탐지, 종합 AI 진단 — 사진 진단 기능과 함께 제거됨, 재추가는 미정.

### Phase 4 — 고급 복구 (31장) — 미착수 (변경 없음)
- [ ] 부분 손상 JPEG 복구, Thumbnail 추출, EXIF 복구, RAW 분석, 복구 가능성 점수화

### MVP에서 명시적으로 제외 (30장) — ⚠️ v1 PRD 원문 그대로, 지금 상태와 일부 안 맞음
AI 이미지 복원, 얼굴 인식, 이미지 내용 분석, ~~자동 사진 분류~~, 클라우드 연동, RAW 복구, 삭제된 파일 복구, 디스크 섹터 단위 복구, 전문 데이터 복구 기능

- **"자동 사진 분류"는 더 이상 제외 항목이 아님** — 위 카테고리 찾기로 실제 구현되어 있음. 이 목록은 원 PRD 30장을 그대로 옮겨온 것이라 갱신하지 않았음 — 다음에 이 항목을 다시 참고할 때는 이 각주부터 볼 것.
- AI 이미지 복원/얼굴 인식/이미지 내용 분석(사진 진단)은 한 번 만들었다가 되돌렸으므로 지금은 이 제외 목록이 다시 맞음(단, "제외"라기보다 "만들었다가 뺌"이 더 정확한 표현).
...
12. **[P3] Phase 2 이후** — 위 P3 섹션 참고. Phase 2(정리)는 사실상 완료, Phase 3(AI)는 대부분 만들었다가 되돌림 — 남은 갭은 "장소 세부 단위/기기정보/스크린샷 자동 묶기/GPS 추론"(PHASE2_사진정리_기획.md) 정도.
