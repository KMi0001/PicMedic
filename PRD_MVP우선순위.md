...
1. **[갭 #4, 버그] 부분 손상 파일 복구 안 되는 문제 수정** — `core/converter.py`에 truncated-image 허용 모드 추가
2. **[갭 #2] 예외 메시지 PRD 문구 정합화** — 권한/잠금/저장공간 케이스별 메시지 및 저장공간 부족 시 배치 중단 로직
3. **[갭 #5, UX] 완전 손상 파일 체크박스 비활성화/경고 강조**
4. **[P2 마무리, 선택사항] 확장자/형식 검색 확장, 파일 크기·날짜 컬럼 추가 후 정렬 지원**
5. **[선택사항] 변환 품질(quality) 조절 UI 노출**
6. **[P3] Phase 2 이후** — 이 문서의 P3 섹션 참고, MVP 완결 후 착수
