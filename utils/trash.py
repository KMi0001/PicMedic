...
파일은 항상 임시휴지통 폴더 바로 아래 평평하게 저장한다 — "정리 실행" 1회에
...
2026-09-10, 사용자 요청으로 앱 설치 폴더 밑 전역 폴더 하나(옛 TRASH_DIR) 대신
"옮기는 파일이 있던 폴더 바로 밑"에 임시휴지통을 만들도록 바꿨다 — 여러 폴더를
한 번에 검사했을 때 어디서 옮겨졌는지 헷갈리지 않고, 복원할 때도 파일 바로
옆이라 직관적이다. 그래서 trash_dir을 전역 상수가 아니라 매번 대상 경로에서
계산한다(trash_dir_for). 매니페스트도 임시휴지통 폴더마다 따로 있다.
...
def trash_dir_for(path: str | Path) -> Path:
    """path가 옮겨질(또는 이미 담겨 있는) 임시휴지통 경로를 돌려준다 — path가
    파일이면 그 파일이 있는 폴더 바로 밑, path 자체가 폴더면 그 폴더 바로
    밑. 폴더를 실제로 만들지는 않는다(호출부가 필요할 때 mkdir)."""
    path = Path(path)
...
def _load_manifest(trash_dir: Path) -> dict:
    manifest_path = _manifest_path(trash_dir)
...
def _save_manifest(trash_dir: Path, manifest: dict) -> None:
...
def move_to_trash(
    ...
    *,
    ...
    """파일 하나를 원래 있던 폴더 바로 밑의 임시휴지통(trash_dir_for)으로
    평평하게 옮기고 최종 경로를 반환한다. group_id/reason/kept_path는
    "정리 실행" 1회로 같이 옮겨진 파일들을 나중에 화면에서 카드 하나로 묶어
    보여주기 위한 정보로, 그 임시휴지통의 매니페스트에만 기록된다(파일
    시스템엔 안 남음). 이름이 이미 있으면 원본을 덮어쓰지 않도록 번호를
    붙인다. restore_from_trash()로 되돌릴 수 있게 원래 경로도 같이 남긴다."""
    path = Path(path)
    original = str(path.resolve())
    trash_dir = trash_dir_for(path)
...
def entry_for(path: str | Path) -> dict | None:
    ...
    kept_path 중 있는 것만). path가 속한 임시휴지통(=path의 부모 폴더)의
    매니페스트에서 찾는다. 없으면 None. 이 기능이 생기기 전에 평평하게
...
def restore_from_trash(path: str | Path) -> Path:
    """임시 휴지통에 있는 파일 하나를 원래 있던 폴더로 되돌린다. path의 부모
    폴더를 그 파일이 속한 임시휴지통으로 본다. 원래 경로를 모르면(매니페스트에
    없음) ValueError. 원래 폴더가 사라졌으면 새로 만들고, 같은 이름 파일이
...
def list_trash(trash_dir: str | Path) -> list[Path]:
    """trash_dir(임시휴지통 폴더 하나)에 있는 파일 목록(평평한 구조 —
    매니페스트 자체는 제외). 존재하지 않으면 빈 목록."""
...
def migrate_group_folders_to_flat(trash_dir: str | Path) -> tuple[int, list[str]]:
    """trash_dir 안에 옛 폴더 구조(<그룹폴더>/파일들 + _사유.txt)로 남아있는
    파일을 지금 구조(trash_dir 바로 아래 평평하게 + 매니페스트에 group_id/
    reason/kept_path)로 옮긴다. 옮길 옛 폴더가 없으면 아무 일도 안 하므로
    여러 번 실행해도 안전하다. 반환값: (옮긴 파일 수, 실패 메시지 목록)."""
    trash_dir = Path(trash_dir)
    if not trash_dir.exists():
        return 0, []
    ...
    for group_dir in old_dirs:
        ...
        for old_path in list(group_dir.iterdir()):
            ...
            new_dest = _unique_dest(trash_dir, old_path.name)
            try:
